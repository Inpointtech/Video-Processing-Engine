...
def trim_video(file: str,
  ...
  """
  temp, file = quick_rename(file, False)
  if output:
    file = output
  os.system(f'ffmpeg -loglevel error -y -ss {start} -i {temp} -vcodec copy '
            f'-acodec copy -t {end} {file}')
...
def trim_num_parts(file: str,
  ...
  Args:
    ...
    return_list: Boolean (default: True) value to return list of all the
                  trimmed files.
  """
  split_part = int(duration(file) / num_parts)
  start = 0
  # Start splitting the videos into 'num_parts' equal parts.
  video_list = []
  for idx in range(1, num_parts + 1):
    print(start)
    trim_video(file, filename(file, idx), start, split_part)
    start += split_part
...
xa = trim_num_parts(
    '/home/xames3/Desktop/video_processing_engine/tests/trimming/original/test_7.mp4', 5) 
print(xa)
